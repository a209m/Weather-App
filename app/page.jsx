...
export default function Home() {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            if (settings.location_enabled && navigator.geolocation) {
                try {
                    ...
                    console.warn("Geolocation failed or timed out, falling back to active city.");
                }
            }

            // Fallback to activeCity if geolocation didn't set data
            if (!weatherData || weatherData.error) {
                const [w, f] = await Promise.all([
                    getWeatherData(activeCity),
                    getForecastData(activeCity)
                ]);
                weatherData = w;
                forecastData = f;
            }
