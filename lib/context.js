...
export function WeatherProvider({ children }) {
    ...
    const [activeCity, setActiveCity] = useState(
        (typeof window !== 'undefined' && localStorage.getItem('weather_settings'))
            ? (JSON.parse(localStorage.getItem('weather_settings')).location_enabled ? '__GEO__' : (localStorage.getItem('active_city') || 'Madrid'))
            : 'Madrid'
    );
    ...
    const updateSettings = (newSettings) => {
        ...
        // If location is toggled ON, reset to geolocation
        if (newSettings.location_enabled === true) {
            setActiveCity('__GEO__');
        }
    };
